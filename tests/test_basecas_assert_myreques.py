...
# """ДЛЯ ТОГО, ЧТОБЫ УПРОСИТЬ ЛОГИКУ СОЗДАНИЯ ASSERTION ДЛЯ ПРОВЕРКИ ТЕСТОВ, МЫ МОЖЕМ ВЫНЕСТИ ВСЕ МЕТОДЫ ПРОВЕРОК В ОТДЕЛЬНЫЙ КЛАСС В ДИРИКТОРИИ lib"""
# """СОЗДАДИМ В ДИРЕКТОРИИ LIB ЕЩЕ ОДИН ФАЙЛ assertions"""


from lib.base_case import BaseCase  # импортировать класс в файл
from lib.assertions import Assertions  # импортируем класс для использования методов assertions

# '''ЧТОБЫ УНАСЛЕДОВАТЬ КЛАСС BaseCase надо открыть круглые скобки и написать BaseCase, а так же

# """чтобы работать с my_requests надо вызвать в файле библиотеку импортировать ее"""
from lib.my_requests import MyRequests  # импортируем my requests
...
class TestUserAuth(BaseCase):
    ...
    def setup(self):
        ...
        # заменяем url 1 значение в self.response1 на URI "/user/login"
        self.response1 = MyRequests.post(
            "/user/login",
    ...
    def test_auth_user(self):
        ...
        # заменяем url первое значение в response2 на URI "/user/auth"
        ...
        # """ВМЕСТО АССЕРТОВ НА ПРОВЕРКУ НАХОЖДЕНИЯ user_id в json файле мы используем Assertions из lib/assertions.py """

        Assertions.assert_json_value_by_name(
            response2,
            "user_id", self.user_id_from_auth_method,
            f"User id from auth method are not equal user id from response1 {self.user_id_from_auth_method}"
        )
    ...
    def test_negative_auth_check(self, condition):  # В ФУНКЦИИ ОБЯЗАТЕЛЬНО УКАЗЫВАТЬ СЛОВО "тест"
        ...
        if condition == "no_cookies":
            response2 = MyRequests.get("/user/auth",  # заменяем url 1 значение в response2 на URI "/user/auth"
                                       # ЗАПРОС ГОВОРИТ О ТОМ АВТОРИЗИРОВАЛИСЬ МЫ,
                                       # ИЛИ НЕ АВТОРИЗИРОВАЛИСЬ
        ...
        # """ЗАМЕНЯЕМ ПРОВЕРКИ user_id на методы из lib/assertions.py"""

        # """ЧЕРЕЗ метод assert_json_value_by_name мы проверям что user_id ==0 в response2, если это не так то отдаем ошибку"""

        Assertions.assert_json_value_by_name(response2, "user_id", 0,
                                             f"Мы прошли авторизацию, несмотря на то что был введен только {condition} {response2.text}")
