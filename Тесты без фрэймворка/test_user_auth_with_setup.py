...
class TestUserAuth:
    ...
    def setup(self):
        ...
        # ТАК ЖЕ ВЫНЕСЕМ ИЗ МЕТОДА test_auth_user ассерты и переменные
        ...
        # """ИЗ ПЕРЕМЕННЫХ МЫ СОЗДАЕМ МЕТОДЫ, КОТОРЫЕ СМОЖЕМ ВЫЗЫВАТЬ В ДРУГИХ ФУНКЦИЯ"""
    ...
    def test_auth_user(self):
        ...
        response2 = requests.get("https://playground.learnqa.ru/api/user/auth",
                                 ...
                                 headers={"x-csrf-token": self.token_x_csrf},  # ПЕРЕДАЕМ ТОКЕН В ЗАПРОС HEADERS
